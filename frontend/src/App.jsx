...
import ClosingSalesView from './components/ClosingSalesView'
...
function App() {
    ...
    useEffect(() => {
        ...
        const {
            ...
            if (session) {
                if (event === 'SIGNED_IN') {
                    window.history.replaceState(null, '', '/SD')
                }
                fetchUserPermissions(session)
            } else {
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-slate-400 mt-4">Cargando permisos...</p>
                </div>
            </div>
        )
    }
            ...
            <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black text-white flex">
                ...
                <main className="flex-1 overflow-y-auto">
                    <Routes>
                        <Route
                            path="/SD"
                            element={<HomeView userPermissions={userPermissions} />}
                        />
                        <Route
                            path="/reports"
                            element={
                        ...
                        <Route
                            path="/closing-sales"
                            element={
                                userPermissions?.can_view_closing_sales ? (
                                    <ClosingSalesView session={session} />
                                ) : (
                                    <Navigate to="/unauthorized" replace />
                                )
                            }
                        ...
                        <Route
                            path="/commissions"
                            element={
                                userPermissions?.can_view_commission ? (
                                    <CommissionView session={session} />
                                ) : (
                                    <Navigate to="/unauthorized" replace />
                                )
                            }
