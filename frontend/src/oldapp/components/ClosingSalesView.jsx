...
export default function ClosingSalesView({ session }) {
    ...
    // Fetch cashes when entity changes
    useEffect(() => {
        fetchCashes()
    }, [entity])
    ...
    const fetchCashes = async () => {
        try {
            const res = await fetch(`http://localhost:8000/api/closingsales/cashes/?entity=${entity}`, {
                headers: { 'Authorization': `Bearer ${session.access_token}` }
            })
            if (res.ok) {
                const data = await res.json()
                setCashes(data)
                setSelectedCashes(data.map(c => c.caja))
            }
        } catch (err) {
            console.error('Error fetching cashes:', err)
        }
    }

    const fetchClosingData = async () => {
        if (!date || selectedChannels.length === 0 || selectedCashes.length === 0) {
            setError('Por favor selecciona fecha, canales y cajas')
            return
        }
        ...
        try {
            ...
            if (!res.ok) throw new Error('Error al obtener datos de cierre')
